...
例えば、コインを何回か投げる試行について考えてみよう。

冪級数 \\(f(x)\\) を「表が \\(k\\) 回出る確率は \\([x^k] f\\) である」ように定義する。
特に全事象を考えると、係数の和は \\(1\\) となる。
...
\\(\big\\{ 0, 1, 2 \big\\}\\) マス動くことを等確率で選ぶロボットを考える。  
冪級数 \\(f(x)\\) を「位置が \\(k\\) である確率は \\([x^k] f\\) である」ように定義する。
...
また、冪級数のメリットとして、\\(x\\) に適切な値を代入した \\(f(x)\\) の値から考察をすることができる。
